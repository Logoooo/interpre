...
@Service("simComService")
public class SimComServiceImpl implements SimComService {
    @Override
    public List similaritySet(double[] goal, double[][] history) {
        Map<Integer,Double> map = new HashMap<Integer, Double>();

        //开始计算
        double[] minArray = getMinArray(history);
        double[] maxArray = getMaxArray(history);

        double[][] standardArray = getStandardizationValue(history,minArray,maxArray);
...
//                System.out.print(" 比重："+standardArray[i][j]);
//            }
//            System.out.println();
//        }
        ...
        //历史案例与目标案例之间的相似度为：
        double[] similitySet = new double[history.length];

        for (int i=0; i<history.length;i++){
            similitySet[i] = computerSimility(history[i],attributeWeight,goal);
